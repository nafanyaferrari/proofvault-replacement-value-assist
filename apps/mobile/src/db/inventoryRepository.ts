...
export async function listInventory(db: SQLiteDatabase) {
  const rows = await db.getAllAsync<ItemRow>('SELECT id,item_name,category,location_text,room,make,model,serial_number,barcode,owner_marking,marking_type,marking_location,marking_notes,distinguishing_features,purchase_date,purchase_price,user_description,notes,user_entered_value,condition,status,archived_at,created_at,updated_at FROM inventory_items WHERE archived_at IS NULL ORDER BY updated_at DESC');
  return rows.map(fromRow);
}

export async function listArchivedInventory(db:SQLiteDatabase){const rows=await db.getAllAsync<ItemRow>('SELECT id,item_name,category,location_text,room,make,model,serial_number,barcode,owner_marking,marking_type,marking_location,marking_notes,distinguishing_features,purchase_date,purchase_price,user_description,notes,user_entered_value,condition,status,archived_at,created_at,updated_at FROM inventory_items WHERE archived_at IS NOT NULL ORDER BY archived_at DESC');return rows.map(fromRow);}
...
export async function saveInventoryItem(db: SQLiteDatabase, draft: InventoryDraft, itemId?: string) {
  ...
  const id=uid('item');
  await db.runAsync('INSERT INTO inventory_items (id,item_name,category,location_text,room,make,model,serial_number,barcode,has_owner_marking,owner_marking,marking_type,marking_location,marking_notes,distinguishing_features,purchase_date,purchase_price,user_description,notes,user_entered_value,condition,status,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',id,draft.itemName.trim(),draft.category.trim(),draft.location.trim(),draft.room.trim()||null,draft.make.trim()||null,draft.model.trim()||null,draft.serialNumber.trim()||null,draft.barcode.trim()||null,draft.ownerMarking.trim()?1:0,draft.ownerMarking.trim()||null,draft.markingType.trim()||null,draft.markingLocation.trim()||null,draft.markingNotes.trim()||null,draft.distinguishingFeatures.trim()||null,draft.purchaseDate.trim()||null,draft.purchasePrice??null,draft.userDescription.trim()||null,draft.notes.trim()||null,draft.userEnteredValue??null,draft.condition,draft.status,now,now);
  return id;
}
