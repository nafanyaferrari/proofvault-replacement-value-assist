...
interface InventoryEditorProps { item?:InventoryItem; initialDraft?:InventoryDraft; assisted?:boolean; locationSuggestions?:string[]; onCancel():void; onSave(draft:InventoryDraft):Promise<void>; }
...
export function InventoryEditor({ item, initialDraft, assisted=false, locationSuggestions=[], onCancel, onSave }: InventoryEditorProps) {
  const [draft, setDraft] = useState<InventoryDraft>(()=>initialDraft??{ itemName:item?.itemName??'', category:item?.category??'', location:item?.location??'', room:item?.room??'', make:item?.make??'', model:item?.model??'', serialNumber:item?.serialNumber??'', barcode:item?.barcode??'', ownerMarking:item?.ownerMarking??'', markingType:item?.markingType??'', markingLocation:item?.markingLocation??'', markingNotes:item?.markingNotes??'', distinguishingFeatures:item?.distinguishingFeatures??'', purchaseDate:item?.purchaseDate??'', purchasePrice:item?.purchasePrice, userDescription:item?.userDescription??'', notes:item?.notes??'', userEnteredValue:item?.userEnteredValue, condition:item?.condition??'unknown', status:item?.status??'normal' });
  ...
  return <Modal visible animationType="slide" presentationStyle="pageSheet" onRequestClose={onCancel}><ScrollView style={styles.safe} contentContainerStyle={styles.page} keyboardShouldPersistTaps="handled">
    ...
    {assisted?<View style={styles.assist}><Text style={styles.assistTitle}>AI-prefilled draft</Text><Text style={styles.assistCopy}>Review the highlighted identity details before saving. Always compare the serial number with the label on the physical item.</Text></View>:null}
