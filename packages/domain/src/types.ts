...
export interface InventoryDraft { itemName:string; category:string; location:string; make:string; model:string; serialNumber:string; ownerMarking:string; markingLocation:string; distinguishingFeatures:string; userDescription:string; userEnteredValue?:number; condition:ItemCondition; }
